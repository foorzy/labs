...
if (isset($_POST['save'])){
    ...
    if($row_cnt > 0) {
		...
		fwrite($f, "-----------------\n");
		fwrite($f, "Добавлено для = ".$ProductID."\n"); // Записываем данные
		fwrite($f, "Количество = ".$Qty."\n"); // Записываем данные
		fwrite($f, "-----------------\n");
...
if (isset($_POST['update'])){
    ...
    $Qty = $_POST['Qty'];
        ...
        if ($mysqli->error) {
            $_SESSION['message'] = "Нельзя изменить код товара, на уже существующий, проверьте правильность ввода данных";
            $_SESSION['msg_type'] = "danger";
